...
namespace IShow.ChooseDishes
{
    public class DishesWayDataService : IDishesWayDataService
    {
        ...
        public List<DischesWay> FindAllDishesWayByTypeId(int wayId)
        {
            ...
            {
                var type = entity.DischesWay.Include(typeof(DischesWayName).Name).Where(d => d.Deleted == 0&&d.DischesWayNameId == wayId).ToList();
                return type;
            }
        }
        /**根据做法id查找做法*/
        ...
        public List<DischesWay> FindDishesWayByCode(string code)
        {
            if (code ==null)
            {
            ...
            using (ChooseDishesEntities entities = new ChooseDishesEntities())
            {
                try
                {
                    var type = entities.DischesWay.Include(typeof(DischesWayName).Name).Where(bt => bt.Code.Equals(code.Trim())).ToList();
                    return type;
                }
                catch (Exception e)
                {
        ...
        public DischesWayName FindDishesWayNameById(int id)
        {
            ...
            using (ChooseDishesEntities entities = new ChooseDishesEntities())
            {
                try
                {
                    var type = entities.DischesWayName.Include(typeof(DischesWay).Name).SingleOrDefault(bt => bt.DischesWayNameId == id&&bt.Deleted==0);
                    return type;
                }
                catch (Exception e)
                {

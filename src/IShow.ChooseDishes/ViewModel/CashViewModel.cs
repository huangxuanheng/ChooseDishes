...
namespace IShow.ChooseDishes.ViewModel
{
    public class CashViewModel : ViewModelBase
    {
        ...
        public RelayCommand AddCash
        {
            get
            {
                return _AddCash ?? (_AddCash = new RelayCommand(() =>
                {
                   ...
                   if (ct != null)
                   {
                       LoadCashTypeBase(0,1);
                       _CashTypeBean = new CashTypeBean();
                       act.Close();
                   }
                   else
                   {
                       MessageBox.Show("新增操作失败!有可能 快捷键有重复 ! 请检查!");
                   }
                }));
            }
        }
        ...
        public ObservableCollection<CashTypeBean> CashTypes { get; set; }
        ...
        public CashType SaveCashType()
        {
            ...
            return _DataService.addCashType(_CashTypeBean.CreateCashType(_CashTypeBean));
        }
        public void LoadCashTypeBase(int CashBaseTypeId,int index)
        {
            List<CashType> array = _DataService.findCashType(0);
            if (index == 0) { 
                CashTypes = new ObservableCollection<CashTypeBean>();
            }
            CashTypes.Clear();
